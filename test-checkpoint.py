...
WEIGHTS_PATH_NO_TOP = 'inception-v4_weights_tf_dim_ordering_tf_kernels_notop.h5'

def prediction(y_pred, predict, total):
    temp = []
    count = 0
    for i in range(len(y_pred)):
        if (y_pred[i] == 0 or y_pred[i] == 1) and count < total:
            temp.append(3)
            count = count + 1
        else:
